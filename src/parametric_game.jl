...
function ParametricGame(;
    ...
    fs = [objectives[i](x, θ) for i in 1:N]
    gs = [equality_constraints[i](x, θ) for i in 1:N]
    hs = [inequality_constraints[i](x, θ) for i in 1:N]
    g̃ = shared_equality_constraint(x, θ)
    h̃ = shared_inequality_constraint(x, θ)

    # Build Lagrangians for all players.
    Ls = [fs[i] - sum(λ[Block(i)]' * gs[i]) - sum(μ[Block(i)]' * hs[i]) - sum(λ̃' * g̃) - sum(μ̃' * h̃) for i in 1:N]

    # Build F = [∇ₓLs, gs, hs, g̃, h̃]'.
    F = Vector{Symbolics.Num}(undef, 0)
    for i in 1:N
        F = [F; Symbolics.gradient(Ls[i], x[Block(i)])]
    end
    for i in 1:N
        F = [F; gs[i]]
    end
    ...
    z̲ = [-Inf * ones(sum(primal_dimensions) + sum(equality_dimensions)); zeros(sum(inequality_dimensions)); -Inf * ones(shared_equality_dimension); zeros(shared_inequality_dimension)]
